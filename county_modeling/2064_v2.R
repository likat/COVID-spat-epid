...
staniters=8000
modellab <- "2064_v2" ## CHANGE THIS 
agegrplab <- "20-64 years" ## CHANGE THIS
death <- readr::read_csv("Fully_Vaccinated_20_64.csv") ## CHANGE THIS
...
# modeldf <- modeldf %>% group_by(county) %>% summarise(mean(mortalityrt))
# ape::Moran.I(modeldf$`mean(mortalityrt)`,W)
