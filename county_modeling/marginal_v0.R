...
modellab <- "marginal_v0" ## CHANGE THIS 
# agegrplab <- "75+ years" ## CHANGE THIS
death <- readr::read_csv("Unvaccinated_Marginal.csv") ## CHANGE THIS
...
p <- 5
ncty = nrow(adjmat)
## CHANGED HERE- marginalize vaccine over age groups
vaccine <- 
  vaccine %>% 
  group_by(county, date) %>% 
  summarise(
    two_dose_count = sum(two_dose_count),
...
modeldf <- 
  ...
  transmute(
    county = county,
    t=t,
    mortalityrt = log((deaths+1)/(unvaccinated+0.001)) ## CHANGE THIS
  ) %>% 
  arrange(county, t)
...
modeldf <- reshape(as.data.frame(modeldf), timevar = "t", idvar="county",direction="wide")
